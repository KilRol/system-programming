...
void swap_words(char **buf, int i, int j)
{
...
void process_file(char *filename)
{
    ...
    int len = words_count(fd);

    char **buf = (char **)malloc(len * sizeof(char *));
    for (int i = 0; i < len; i++)
    {
        buf[i] = (char *)malloc(BUFSIZ * sizeof(char));
    }
    for (int i = 0; i < len; i++)
    {
        fscanf(fd, "%s", buf[i]);
...
{
    logbuf = fopen("swap_neighboring_words.log", "w");
    if (argc != 4)
    {
    ...
    } while (sem_state != 0 || processing != 0);
